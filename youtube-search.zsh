...
youtube-search () {
    ...
    if [[ -n "$use_thumbnails" ]]; then
        ...
        uv tool run -q yt-dlp@latest --no-warnings --skip-download --quiet \
            "${extra_opts[@]}" --print "$thumb_format" "$source" \
        | while IFS=$'\t' read -r thumbnail url id date views duration uploader title; do
            local tmp=$(mktemp /tmp/yt-thumb-XXXXXX)
            curl -fsSL "$thumbnail" -o "$tmp" 2>/dev/null || { rm -f "$tmp"; continue; }
            local img_dims=($(sips -g pixelWidth -g pixelHeight "$tmp" 2>/dev/null | awk '/pixel/{print $2}'))
            ...
            timg -g "${thumb_cols}x${thumb_rows}" "$tmp"
            rm -f "$tmp"
